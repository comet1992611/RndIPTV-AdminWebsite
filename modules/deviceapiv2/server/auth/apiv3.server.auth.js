                ...
                models.login_data.findOne({
                    ...
                    if(result) {
                        ...
                        if(auth_obj.username !== 'guest'){
                            ...
                            var hashed_once = authenticationHandler.encryptPassword(result.username, result.salt);

                            if(result.account_lock) {
                                response.send_res(req, res, [], 703, -1, 'ACCOUNT_LOCK_DESCRIPTION', 'ACCOUNT_LOCK_DATA', 'no-store');
                            }
                            else if( (auth_obj.password.replace(/ /g, "+")+"==" !== hashed_once) && (auth_obj.password.replace(/ /g, "+") !== hashed_once) ) {
